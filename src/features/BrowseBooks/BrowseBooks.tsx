// BrowseBooks.tsx
import React from 'react';
import { useSelector } from 'react-redux';
import { Card, Row, Col } from 'react-bootstrap';
import { RootState } from '../../app/store';
import { Link } from "react-router-dom";
import { IBookFormState } from '../../interface';
const url = 'data:image/png;base64,'

const BrowseBooks: React.FC = () => {
  const books = useSelector((state: RootState) => state.browseBooks.books);

  return (
    <div>
      <h2>Browse Books</h2>
      <Row>
        {books.map((book:IBookFormState, index:number) => (
            <Col key={index} xs={12} sm={6} md={4} lg={3}>
              <Link style={{textDecoration:'none'}} to={`${book.id}`} key={book.id}>
            <Card style={{ marginBottom: '16px' }}>
              <Card.Img variant="top" src={url} alt="Book Cover" />
              <Card.Body>
                <Card.Title>{book.bookName}</Card.Title>
                <Card.Text>
                  <strong>ISBN:</strong> {book.isbn} <br />
                  <strong>Category:</strong> {book.category} <br />
                  <strong>Row No:</strong> {book.rowNo} <br />
                  <strong>Book Count:</strong> {book.bookCount} <br />
                  <strong>Cost:</strong> {book.cost} <br />
                  <strong>Availability:</strong> {book.availability ? 'Available' : 'Not Available'}
                </Card.Text>
              </Card.Body>
            </Card>
          </Link>
          </Col>

        ))}
      </Row>
    </div>
  );
};

export default BrowseBooks;
